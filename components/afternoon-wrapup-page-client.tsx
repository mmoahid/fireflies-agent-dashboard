...
import { saveObjective, updateAgendaProgressNote, updateAgendaStatus } from "@/lib/actions"
import { toast } from "sonner"

interface AfternoonWrapupPageClientProps {
  objective: CompanyObjectiveDTO
  items: AgendaItemDTO[]
}

export function AfternoonWrapupPageClient({ objective, items: initialItems }: AfternoonWrapupPageClientProps) {
  ...
  const handleProgressUpdate = (id: string, note: string) => {
    ...
    startTransition(async () => {
      try {
        await updateAgendaProgressNote(id, note)
      } catch (err) {
        const message = err instanceof Error ? err.message : "Failed to save progress note"
        toast.error("Could not save note", { description: message })
      }
    })
  }

  const incompleteItems = useMemo(() => items.filter((i) => i.status !== "completed"), [items])
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button
          className="gap-2"
          disabled={isPending}
          onClick={() => toast.message("Notes are local for now", { description: "Wire DailySync persistence next." })}
        >
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <SyncAgenda type="afternoon" items={items} onStatusChange={handleStatusChange} onProgressUpdate={handleProgressUpdate} />
